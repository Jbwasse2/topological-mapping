...
class Siamese(nn.Module):
    ...
    def __init__(self, context=10):
        ...
        self.lstm1 = nn.LSTM(input_size=3136, hidden_size=self.hidden_size,
                             batch_first=True, num_layers=self.lstm_num_layers, dropout=0.0)
        self.conv1 = conv(True, 1024, 256, kernel_size=1,
...
#        ret = ret.view(-1, ret.shape[1], ret.shape[2] * ret.shape[3] * ret.shape[4])
    ...
    def encode(self, embedding):
        x = embedding.view(embedding.shape[0] * self.context,
                           embedding.shape[2], embedding.shape[3], embedding.shape[4])
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        x = x.view(embedding.shape[0], self.context,
...
class SiameseDeepVO(nn.Module):
    def __init__(self, context=10):
        ...
        self.lstm_num_layers = 2
        self.hidden_size = 128
        self.lstm1 = nn.LSTM(input_size=16384, hidden_size=self.hidden_size,
                             batch_first=True, num_layers=self.lstm_num_layers, dropout=0.0)
        self.conv1 = conv(self.batchNorm,   6,   64,
                          kernel_size=7, stride=2, dropout=True)
        self.conv2 = conv(self.batchNorm,  64,  128,
                          kernel_size=5, stride=2, dropout=True)
        self.conv3 = conv(self.batchNorm, 128,  256,
                          kernel_size=5, stride=2, dropout=True)
        self.conv3_1 = conv(self.batchNorm, 256,  256,
                            kernel_size=3, stride=1, dropout=True)
        self.conv4 = conv(self.batchNorm, 256,  512,
                          kernel_size=3, stride=2, dropout=True)
        ...
        self.conv5_1 = conv(self.batchNorm, 512,  512,
                            kernel_size=3, stride=1, dropout=True)
        self.conv6 = conv(self.batchNorm, 512, 1024,
                          kernel_size=3, stride=2, dropout=True)
        self.fc5 = nn.Linear(128, 2)
        # Pose
        self.rnn_drop_out = nn.Dropout(0.5)
        self.linearPose = nn.Linear(in_features=self.hidden_size, out_features=6)
        self.linearSim = nn.Linear(in_features=self.hidden_size + 6, out_features=2)
...
#        ret = ret.view(-1, ret.shape[1], ret.shape[2] * ret.shape[3] * ret.shape[4])
    ...
    def forward(self, x1, x2):
        x = self.imageEncode(torch.cat([x1,x2],dim=2))
        x = x.view(x.shape[0], x.shape[1], -1)
        x, hidden = self.lstm1(x, self.hidden)
        self.hidden = hidden
#        x = x.reshape(x.shape[0], -1)
        x = x[:,-1,:]
        pose = self.linearPose(x)
