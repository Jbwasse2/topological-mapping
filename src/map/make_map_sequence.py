...
def add_trajs_to_graph(G, trajectories, traj_ind, device, similarity, episodes, sim):
    for trajectory_count, (trajectory_count_episode, trajectory) in tqdm(
        ...
        for frame_count, frame in enumerate(trajectory):
            ...
            for node in list(G.nodes):
                ...
                results = estimate_edge_len_SLAM(edge)
...
def connect_graph_trajectories(
    G, trajectories, traj_ind, device, similarity, episodes, sim=None
):
    counter = 0
