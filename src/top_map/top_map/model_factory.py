...
def model_12env_v2_future_pair_proximity_z0228(device="cuda", **kwargs):
    ...
    follow_thres = 0.92
    motion_policy = ControllerMultiframeDst(
        controller_models_dir
        + "multiframe_dst/gtwp-normwp-farwp-jitter-weightedloss-checkwp-nf6-interval3-dmax3-z0228-model.8",
        device=device,
    )
    sparsifier = TrajSparsifierMultiframeDst(
        reachability_models_dir
        + "multiframe_dst/farwp-jitter-difftrajprob0.4-z0228-model.7",
        device=device,
    )
    follower = TrajectoryFollowerMultiFrameDstBothProximity(
        sparsifier, motion_policy, search_thres, follow_thres
    )
    ret = {
